...
| Local trace-job recovery | Implemented for the single-process prototype | Job records retain the case and request payload. Jobs interrupted by restart are explicitly marked `INTERRUPTED` and may be retried; multi-host queue execution remains deployment work. |
...
| Defensible action centre | Implemented | Bounded candidate amount, adversarial evidence checks, next-best-evidence actions, and reviewed local VASP readiness profiles are computed from the immutable result. It proposes local preparation only and never executes a freeze. |
| Public-risk intelligence pack | Implemented as review queue | A local builder creates unreviewed source-preserving OFAC SDN cryptocurrency-address records. It does not claim VASP ownership. |
| Cross-chain continuation | Evidence workflow implemented | A reviewed route and exact shared protocol message ID are required. Normalized source/destination events can be extracted from retained raw-evidence artifacts and resolved only when their exact IDs match. The built-in Wormhole EVM collector decodes source VAA identifiers from retained Core Contract receipt logs; destination proof remains required. |
